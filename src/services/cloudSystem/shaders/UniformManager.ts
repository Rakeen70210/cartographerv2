...
export class UniformManager {
  ...
  setUniform(name: string, value: number | number[] | Float32Array, type?: string): void {
    ...
    if (!location) {
      // Silently skip - not all uniforms are present in all shaders
      return;
    }
  ...
  setTextureUniform(name: string, texture: WebGLTexture, textureUnit: number = 0): void {
    const location = this.uniformLocations[name];
    if (!location) {
      // Silently skip - texture uniform may not be present in shader
      return;
    }
  ...
  private bindUniform(location: WebGLUniformLocation, value: number | number[] | Float32Array, type: string): void {
    // Wrap in try-catch to suppress WebGL INVALID_OPERATION warnings
    // These can occur when the associated program is not currently bound
    try {
      switch (type) {
        case 'float':
      ...
      // Silently ignore WebGL errors - uniform location may not be from current program
    }
  }
